...
class TuyaHomeMemberModule(reactContext: ReactApplicationContext?) : ReactContextBaseJavaModule(reactContext!!) {
    ...
    /* 查询Home下面的成员列表 */
    @ReactMethod
    fun processInvitation(params: ReadableMap, promise: Promise) {
        if (ReactParamsCheck.checkParams(arrayOf(Constant.HOMEID,Constant.ACTION),params)) {
            TuyaHomeSdk.getMemberInstance().processInvitation(params.getDouble(Constant.HOMEID).toLong(),params.getBoolean(Constant.ACTION), getIResultCallback(promise))
        }
    }
}