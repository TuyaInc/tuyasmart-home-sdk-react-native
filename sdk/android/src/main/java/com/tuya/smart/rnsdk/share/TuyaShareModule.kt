...
class TuyaShareModule(reactContext: ReactApplicationContext?) : ReactContextBaseJavaModule(reactContext!!) {
    ...
    fun addShareWithHomeId(params: ReadableMap, promise: Promise) {
        if (ReactParamsCheck.checkParams(arrayOf(HOMEID, COUNTRYCODE, USERACCOUNT, DEVIDS), params)) {
            ...
            val length = params.getArray(Constant.DEVIDS)!!.size()
    ...
    /*查询收到指定用户共享的信息*/
    @ReactMethod
    fun enableDevShare(params: ReadableMap, promise: Promise) {
        if (ReactParamsCheck.checkParams(arrayOf(MEMBERID, DEVID), params)) {
            TuyaHomeSdk.getDeviceShareInstance().enableDevShare(params.getString(DEVID),params.getDouble(MEMBERID).toLong(), getIResultCallback(promise))
        }
    }

    /*可分享给未注册用户*/
    @ReactMethod
    fun inviteShare(params: ReadableMap) {
        if (ReactParamsCheck.checkParams(arrayOf(MEMBERID), params)) {
    ...
    /* 邀请分享确认*/
    @ReactMethod
    fun confirmShareInviteShare(params: ReadableMap) {
        if (ReactParamsCheck.checkParams(arrayOf(MEMBERID), params)) {
    ...
    private fun getITuyaResultCallback(promise: Promise):  ITuyaResultCallback<List<SharedUserInfoBean>> {
    ...
    private fun getITuyaResultCallbackSingle(promise: Promise):  ITuyaResultCallback<SharedUserInfoBean> {
    ...
    private fun getITuyaResultShareSentUserDetailBeanCallback(promise: Promise):  ITuyaResultCallback<ShareSentUserDetailBean> {
